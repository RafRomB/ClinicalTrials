...
# 1. [Successful Drug Combination Studies - FDA Approval Notifications](https://www.fda.gov/drugs/resources-information-approved-drugs/oncology-cancerhematologic-malignancies-approval-notifications)
...
knitr::kable(retrieve_metrics(data = approval_notifications_test, reference = "manual_eval", models = c("qwen3_8b", "deepseek_r1_8b", "phi4", "ensemble"))
...
The results show a high agreement between the manual and LLM classification:
...
knitr::kable(retrieve_metrics(data = sample_protocolSection_lmm, reference = "manual_eval", models = c("qwen3_8b", "deepseek_r1_8b", "phi4", "ensemble"))
              %>% dplyr::mutate(across(where(is.numeric), round, 3)), caption = "'ensemble' refers to the result corresponding to the majority vote of the three different LLMs.")
...
```{r, message=FALSE, fig.width=16, fig.height=6}
...
barplot_from_table <- function(data, X = NULL, Y, X_name = "X", Y_name = "Count") {
  # Deal with NAs
...
```{r, message=FALSE, fig.width=28, fig.height=8, out.width="100%"}
...
From the 201 approval notifications/171 clinical trials, **6 were not present** in the final list of retrieved clinical studies:
...
```{r,  message=FALSE, fig.width=20, fig.height=8, out.width="100%"}
...
for (v in var_col) {
  ...
  p <- barplot_from_table(data = df,
                          Y = v,
                          X_name = v) + labs(title = paste(v, "- Approved Clinical Trials")) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  p_list <- c(p_list, p)
}
...
# 3. Definition of Non-Successful Drug Combinations

## 3.1 DesignPrimaryPurpose

**Most of the studies' design primary purpose is _TREATMENT_.**
...
In the list of successful combinations, only two have a different primary purpose:

1. **[NCT01743131](https://clinicaltrials.gov/study/NCT01743131): PREVENTION**

[FDA approves abatacept for prophylaxis of acute graft versus host disease](https://www.fda.gov/drugs/resources-information-approved-drugs/fda-approves-abatacept-prophylaxis-acute-graft-versus-host-disease). 

This is a GVHD prophylaxis study in the transplant setting. The regimen components here—abatacept, cyclosporine/tacrolimus (calcineurin inhibitors), and methotrexate (used here as immunosuppression)—are being used for immunoprophylaxis, not as a systemic anticancer combination regimen.

2. **[NCT02465060](https://clinicaltrials.gov/study/NCT02465060): OTHER**
...
The approval specifically mentions the 'Experimental Subprotocol H' (BRAF V600E/R/K/D mutation). Patients with BRAF V600E/R/K/D mutation receive dabrafenib PO BID and trametinib PO QD on days 1-28. It is a drug combination study, but with other experimental subprotocols that are single drug. Importantly, NCT02465060 is just one of the studies referenced in the approval, which also references other two additional clinical trials.

**Keeping only studies with DesignPrimaryPurpose:TREATMENT**
...
It might be a good way to distinguish those studies that have stopped due to efficacy/safety reasons. 
The use of just the overall recruitment status or the phase does not seem to be a robust way to classify the studies in non-successful, given that some approvals are granted for studies that are still active or in phase 2.

```{r, message=FALSE, warning=FALSE, fig.width=20, fig.height=14, fig.cap="OverallStatus and Phase in studies with text in the 'WhyStopped' field and in approved clinical trials"}
...
The **3151 studies** that provided some information for the termination/suspension/withdrawal of the studie were further analyzed. The list of studies were again queried to 3 LLMs (qwen3:14b, deepseek-r1:8b, phi4) to classify the reason of study termination/suspension/withdrawal into **_efficacy/safety/nonclinical_**. The following definitions were used to classify the reason for study termination/suspension/withdrawal:
...
An initial random sample containing 100 clinical trials was manually labeled by an expert blinded to the model output to evaluate the performance of the models. The results show a high agreement between the manual and LLM classification:
...
protocolSection_WhyStopped_sample[c("qwen3_14b",
                             ...
                             "manual_eval")] <- lapply(protocolSection_WhyStopped_sample[c("qwen3_14b",
                                                                                    ...
                                                                                    "manual_eval")], function(x) {
                                                                                      x <- as.character(x)
                                                                                      x[!x %in% c("efficacy", "safety", "nonclinical")] <- "nonclinical" # Replace NAs or NULL with nonclinical
...
knitr::kable(
  retrieve_metrics(
    data = protocolSection_WhyStopped_sample,
...
**Results of LLM Classification of WhyStopped**

```{r, fig.height=12, fig.width=12, out.width="60%"}
...
df$ensemble[!df$ensemble %in%  c("efficacy", "safety", "nonclinical")] <- NA
...
Studies without label:

- 2 studies were terminated due to "toxicity and lack of efficacy".
- 1 study was terminated due to "inability to achieve and optimize a dose that could provide a positive benefit risk profile".

All of them were manually labelled as terminated due to "safety".


## 3.3 Final List of Non-Successful Clinical Trials

![Flow diagram of non-successful clinical trials filtering.](../www/NonSuccessful_Filtering.drawio.png)


**Overall Status and Phase in Successful and Non-successful Clinical Trials**

```{r, warning=FALSE, fig.height=18, fig.width=32, out.width="100%"}

protocolSection_WhyStopped_llm$ensemble[!protocolSection_WhyStopped_llm$ensemble %in%  c("efficacy", "safety", "nonclinical")] <- "safety"

protocolSection_WhyStopped_llm <- protocolSection_WhyStopped_llm %>% filter(ensemble != "nonclinical")
